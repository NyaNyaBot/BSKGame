...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, AskQuestion
---
...
Check:
- **Engine configured?** Read `.cursor/skills/ccgs-studio/references/docs/technical-preferences.md`. If the Engine field contains `[TO BE CONFIGURED]`, the engine is not set.
...
This is the first thing the user sees. Use `AskQuestion` with these exact options so the user can click rather than type:
