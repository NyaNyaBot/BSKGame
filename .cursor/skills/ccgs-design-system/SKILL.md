...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, Edit, Task, AskQuestion, TodoWrite
---
...
2. If it exists: read it, find the highest-priority system with status "Not Started" or equivalent, and use `AskQuestion`:
...
- Read `.cursor/skills/ccgs-studio/references/docs/technical-preferences.md` to identify the engine and version
...
Use the template structure from `.cursor/skills/ccgs-studio/references/templates/game-design-document.md`:
...
**Framing question (ask BEFORE drafting)**: Use `AskQuestion`:
...
Before finalizing the GDD, spawn `creative-director` via Task using gate **CD-GDD-ALIGN** (`.cursor/skills/ccgs-studio/references/docs/director-gates.md`).
