...
- If it exists: read it, find the first context (system/level/character) with any asset at status "Needed" but no spec file written yet, and use `AskQuestion`:
...
- **Technical preferences**: Read `.cursor/skills/ccgs-studio/references/docs/technical-preferences.md` — extract performance budgets and naming conventions.
...
Present the full identified list to the user. Use `AskQuestion`:
...
After presenting all specs, use `AskQuestion`:
