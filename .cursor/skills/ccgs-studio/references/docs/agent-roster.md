...
`.cursor/skills/ccgs-studio/references/agents/`. Use the agent best suited to the task at hand. When a task
spans multiple domains, the coordinating agent (usually `producer` or the
domain lead) should delegate to specialists.

## Cursor Model ID Mapping

| CCGS tier | Cursor model ID |
|-----------|-----------------|
...
Run in **main chat** or spawn Task with `model: "claude-4.6-opus-high-thinking"` for high-stakes synthesis.
...
Delegate via Task with `model: "claude-4.6-sonnet-medium-thinking"` unless the user explicitly keeps the work in main chat.
