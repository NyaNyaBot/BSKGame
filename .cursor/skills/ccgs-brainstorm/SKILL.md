...
user-invocable: true
allowed-tools: Read, Glob, Grep, Write, WebSearch, Task, AskQuestion
---
...
1. **Parse the argument** for an optional genre/theme hint (e.g., `roguelike`,
   ...
   See `.cursor/skills/ccgs-studio/references/docs/director-gates.md` for the full check pattern.
...
**Pillar confirmation**: After presenting the full pillar set, use `AskQuestion`:
...
If the user selects B, C, or D, make the revision, then use `AskQuestion` again:
...
- **`creative-director`** — gate **CD-PILLARS** (`.cursor/skills/ccgs-studio/references/docs/director-gates.md`)
  Pass: full pillar set with design tests, anti-pillars, core fantasy, unique hook.

- **`art-director`** — gate **AD-CONCEPT-VISUAL** (`.cursor/skills/ccgs-studio/references/docs/director-gates.md`)
...
- **Target platform**: Use `AskQuestion` — "What platforms are you targeting for this game?"
...
- **Engine experience**: Use `AskQuestion` — "Do you already have an engine you work in?"
...
**After identifying biggest technical risks, spawn `technical-director` via Task using gate TD-FEASIBILITY (`.cursor/skills/ccgs-studio/references/docs/director-gates.md`) before scope tiers are defined.**
...
**After scope tiers are defined, spawn `producer` via Task using gate PR-SCOPE (`.cursor/skills/ccgs-studio/references/docs/director-gates.md`).**
...
4. **Generate the game concept document** using the template at
   `.cursor/skills/ccgs-studio/references/templates/game-concept.md`. Fill in ALL sections from the
...
5. Use `AskQuestion` for write approval:
...
After revising, show the updated section as a diff or clear before/after, then use `AskQuestion` — "Ready to write the updated concept document?"
Options: `[A] Yes — write it` / `[B] Revise another section`
Repeat until the user selects [A].

If yes, generate the document using the template at `.cursor/skills/ccgs-studio/references/templates/game-concept.md`, fill in ALL sections from the brainstorm conversation, and write the file, creating directories as needed.
